...
class ElectronicsCategoryActivity : AppCompatActivity() {
    ...
    private fun fetchItemsByCategory(category: String) {
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                e.printStackTrace()
                runOnUiThread {
                    Toast.makeText(this@ElectronicsCategoryActivity, "Failed to load items", Toast.LENGTH_SHORT).show()
                }
            }
            ...
            override fun onResponse(call: Call, response: Response) {
                response.body?.let { responseBody ->
                    ...
                    if (jsonResponse.has("items")) {
                        ...
                        runOnUiThread {
                            Toast.makeText(this@ElectronicsCategoryActivity, "No items found for category", Toast.LENGTH_SHORT).show()
                        }
                    }
                }
            }
    ...
    inner class ElectronicsItemAdapter(private val context: ElectronicsCategoryActivity, private val items: List<Item>) :
        BaseAdapter() {
